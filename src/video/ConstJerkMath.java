...
public class ConstJerkMath extends JPanel implements MouseListener, ActionListener, KeyListener{
	...
	public static void main(String[] args) {
		ConstJerkMath drive = new ConstJerkMath();
	}
	
	public ConstJerkMath() {
		...
		if (i != path.length) {
			switch (rc){
				...
				case followTraj:
					if (t > 3) {
						...
						l = new ConstJerkLocalizer(path[i],n);
						currPoseTimer = 0;
					...
					l = new ConstJerkLocalizer(new cubicSpline(path[i],t/3.0),n);
			...
			if (t > 0.5 && n < 100) {
				...
				for (int k = 0; k < path.length; k ++) {
					ConstJerkLocalizer f = new ConstJerkLocalizer(path[k],n);
