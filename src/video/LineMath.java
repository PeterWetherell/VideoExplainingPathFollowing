...
public class LineMath extends JPanel implements MouseListener, ActionListener, KeyListener{
		...
		if (i != path.length) {
			switch (rc){
				...
				case followTraj:
					if (t > 3) {
						...
						l = new LinearLocalizer(path[i],n);
						currPoseTimer = 0;
					...
					l = new LinearLocalizer(new cubicSpline(path[i],t/3.0),n);
			...
			if (t > 0.5 && n < 100) {
				...
				for (int k = 0; k < path.length; k ++) {
					LinearLocalizer f = new LinearLocalizer(path[k],n);
